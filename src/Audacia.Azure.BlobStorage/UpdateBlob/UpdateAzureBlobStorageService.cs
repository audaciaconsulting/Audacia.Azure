...
/// </summary>
[SuppressMessage("Maintainability", "AV1554:Method contains optional parameter in type hierarchy", Justification = "Allows common pattern of optional cancellation token parameter in interface methods.")]
public class UpdateAzureBlobStorageService : BaseAzureUpdateStorageService, IUpdateAzureBlobStorageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (command != null)
        {
            _ = await ContainerChecksAsync(command.ContainerName, command.DoesContainerExist, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (blobExists.Value)
        {
            using (await blobClient.DeleteAsync(cancellationToken: cancellationToken).ConfigureAwait(false))
            {
                try
                {
                    _ = await blobClient.UploadAsync(blobData, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        using (await blobClient.DeleteAsync(cancellationToken: cancellationToken).ConfigureAwait(false))
        {
            ...
            try
            {
                _ = await blobClient.UploadAsync(ms, cancellationToken).ConfigureAwait(false);
                Logger.LogInformation(
                    $"Upload blob data for blob: {command.BlobName} to container: {command.ContainerName}");
                return true;
            }
