...
using Audacia.Azure.Demo.Models.Requests.Blob;
...
[ApiController]
[Route("[controller]")]
public class WeatherForecastController(IAddAzureBlobStorageService addAzureBlobStorageService,
...
{
    private readonly IGetAzureProtectedBlobStorageService _getAzureProtectedBlobStorageService = getAzureProtectedBlobStorageService;

    private readonly IAddAzureBlobStorageService _addAzureBlobStorageService = addAzureBlobStorageService;

    private readonly IUpdateAzureBlobStorageService _updateAzureBlobStorageService = updateAzureBlobStorageService;

    private readonly IDeleteAzureBlobStorageService _deleteAzureBlobStorageService = deleteAzureBlobStorageService;
...
#pragma warning restore CA1031
    ...
    public async Task<IActionResult> Post([FromForm] AddBlobRequest addBlobRequest, CancellationToken cancellationToken = default)
    {
        ...
        var uniqueBlobName = $"{Guid.NewGuid()}.{fileExtension[^1]}";
        ...
        return addBlobResult ? Ok(uniqueBlobName) : BadRequest("Failed to add blob");
    }
    ...
    public async Task<IActionResult> Put([FromForm] UpdateBlobRequest updateBlobRequest, CancellationToken cancellationToken = default)
    {
        ...
        return updateBlobResult ? Ok() : BadRequest("Failed to update blob");
    }
    ...
    public async Task<IActionResult> Delete([FromForm] DeleteBlobRequest deleteBlobRequest, CancellationToken cancellationToken = default)
    {
        ...
        return deleteBlobResult ? Ok() : BadRequest("Failed to delete blob");
    }
}
